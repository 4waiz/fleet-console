...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "relative z-0 border-b transition-all duration-200 hover:bg-muted/60 hover:shadow-[inset_0_0_0_1px_hsl(var(--border))] data-[state=selected]:bg-muted",
        className,
      )}
      {...props}
    />
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-12 whitespace-nowrap px-4 text-left align-middle text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground",
        className,
      )}
      {...props}
    />
